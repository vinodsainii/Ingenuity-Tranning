...
function draw() {
  ...
  if (gameState == "PLAY") {
    ...
    // Collect Coins
    for (var i = 0; i < dragonsGroup.length; i++) {
      var temp = dragonsGroup.get(i);
      if (temp.isTouching(mario)) {
        mario.scale = 0.1;
        //Coin Sound
        coinSound.play();
...
function generateDragons() {
  if (frameCount % 250 === 0) {
    var dragon = createSprite(1100, 110, 35, 11);
    dragon.y = Math.round(random(70, 320));
    dragon.addAnimation("coin", dragonImage);
    dragon.scale = 0.2 ;
    dragon.velocityX = -10;
